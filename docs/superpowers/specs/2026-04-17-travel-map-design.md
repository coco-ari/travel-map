# 旅游地图 — 设计文档

## 概述
一个移动端优先的网页应用，打开后显示以用户当前位置为中心的地图，用户可以在地图上标记想去的店铺/餐厅，并通过管理后台列表管理这些店铺数据。数据持久化在服务端的 SQLite 中。

## 技术栈
- **前端**: 原生 JavaScript + Leaflet.js（OpenStreetMap 瓦片，无需 API Key）
- **后端**: Node.js + Express
- **数据库**: SQLite（单表）
- **部署**: 腾讯云 VPS 公网 IP，使用 pm2/systemd 管理进程

## 系统架构

```
┌─────────────────────────────────────────────┐
│                 手机浏览器                    │
│  ┌───────────────────────────────────────┐   │
│  │          Leaflet.js 地图               │   │
│  │   (OpenStreetMap 免费瓦片, 无需Key)     │   │
│  │   - 显示当前位置 (浏览器GPS定位)         │   │
│  │   - 地图标记店铺                        │   │
│  │   - 点击地图添加店铺                    │   │
│  │   - 弹出表单输入店名                    │   │
│  └───────────────────────────────────────┘   │
│  ┌───────────────────────────────────────┐   │
│  │         店铺管理列表 (后台)              │   │
│  │   - 新增 / 删除店铺                    │   │
│  └───────────────────────────────────────┘   │
│         ↕ HTTP REST API                      │
└─────────────────┬───────────────────────────┘
                  │
┌─────────────────┴───────────────────────────┐
│            Node.js + Express                 │
│  ┌─────────┐  ┌──────────┐  ┌────────────┐  │
│  │GET /shops│ │POST/shops│ │DELETE/shops│  │
│  └─────────┘  └──────────┘  └────────────┘  │
│         ↕                                    │
│  ┌───────────────────────────────────────┐   │
│  │            SQLite (数据库)              │   │
│  │  shop: id, name, lat, lng, status,     │   │
│  │         created_at                     │   │
│  └───────────────────────────────────────┘   │
└─────────────────────────────────────────────┘
```

- 前后端一体：Express 直接提供静态 HTML/JS 文件
- 无认证/登录（个人自用工具）

## 数据库表结构

```sql
CREATE TABLE shops (
  id          INTEGER PRIMARY KEY,
  name        TEXT NOT NULL,       -- 店名
  lat         REAL NOT NULL,       -- 纬度
  lng         REAL NOT NULL,       -- 经度
  status      TEXT DEFAULT 'unvisited',  -- 状态：'visited'（已去）| 'unvisited'（未去）
  created_at  DATETIME DEFAULT CURRENT_TIMESTAMP  -- 创建时间
);
```

## API 接口

```
GET    /api/shops              — 获取所有店铺（支持 ?status=unvisited 过滤）
POST   /api/shops              — 新增店铺 {name, lat, lng}
DELETE /api/shops/:id          — 删除店铺
PATCH  /api/shops/:id/status   — 更新状态 {status: "visited" | "unvisited"}
```

## 页面路由

```
GET  /          — 地图主页
GET  /admin     — 店铺管理列表
```

## 前端设计

### 页面一：地图主页（默认首页）

- 页面加载时通过 `navigator.geolocation` 请求浏览器 GPS 定位
- 地图以当前位置为中心，显示蓝色定位标记
- 已有的店铺以标记形式展示在地图上
- **添加店铺**: 点击"添加"按钮后点击地图位置 → 弹出输入框填写店名 → 确认 → 新标记出现
- **店铺标记气泡**:
  ```
  ┌─────────────────────────┐
  │        店铺名称            │
  │                         │
  │  [已吃]  [详情]  [导航]    │
  └─────────────────────────┘
  ```
  - **已吃**: 标记该店铺为"已去过"，标记变灰
  - **详情**: 弹出卡片显示店名、经纬度坐标、添加时间、状态
  - **导航**: 通过 URL Scheme 直接唤起高德地图 `amapuri://route/plan/?slat=当前&slon=当前&dlat=目标&dlon=目标&dev=0`，未安装则 fallback 到浏览器地图导航
- **默认过滤**: 仅显示"未去"状态的店铺
- **附近高亮**: 距离当前位置 2km 内未去的店铺，标记放大并显示橙色，其余未去店铺显示蓝色
- **"显示全部"开关**: 打开后显示已去的店铺（灰色标记）
- 右上角"列表"图标 → 进入管理页面

### 页面二：店铺管理后台

- 卡片/列表形式展示所有店铺，按创建时间倒序排列
- 每条显示：店名、经纬度、缩略地图
- 操作：删除（确认弹窗）
- 顶部"返回地图"按钮
- "手动添加"按钮：手动输入坐标和店名

## UI 风格 — 微信风格

整体采用微信（WeChat）视觉风格，移动端原生体验感。

**配色方案：**
- 主色：`#07C160`（微信绿）
- 背景色：`#EDEDED`（浅灰）
- 卡片背景：`#FFFFFF`（纯白）
- 文字主色：`#333333`，次要文字：`#999999`
- 分割线：`#E5E5E5`

**组件风格：**
- 导航栏：白色背景 + 底部 `#E5E5E5` 分割线，标题居中，操作按钮在左右两侧
- 按钮：圆角 `6px`，微信绿背景白字，禁用态灰色
- 卡片：白色背景，无阴影或极浅阴影，内边距 `12px`
- 列表项：白色背景，底部 `1px` 分割线，左对齐文字
- 弹窗：居中模态框，圆角 `12px`，底部操作按钮（微信 ActionSheet 风格）
- 输入框：白色圆角背景，底部边框聚焦时变绿

**地图标记风格：**
- 未去店铺（2km 内）：橙色圆点标记，较大尺寸
- 未去店铺（2km 外）：微信绿圆点标记，常规尺寸
- 已去店铺：灰色圆点标记，较小尺寸
- 当前位置：蓝色脉冲圆点（类似微信共享位置）

**交互细节：**
- 点击列表项有微信原生 tap 反馈色（浅灰）
- 删除操作使用底部弹出 ActionSheet 确认
- 页面切换使用微信风格的从右向左滑动过渡

## 部署方案
- 腾讯云 VPS 公网 IP
- 进程管理：pm2 或 systemd
- Nginx 反向代理（可选）
